...
Баннеры автоматически вставляются перед каждым блоком контента (начиная со второго):

- **Перед `short_answer`** → Баннер 2 (`{lang}-google-ads-2`)
- **Перед `key_points_json`** → Баннер 3 (`{lang}-own-ads`)
- **Перед `examples_json`** → Баннер 4 (`{lang}-google-ads-3`)
...
3. `short_answer` - AI-friendly контент-блоки (для “цитируемости”) - (1–2 предложения: “что делает инструмент”)
4. `key_points_json` - (список 5–10 буллетов: “что умеет / ограничения / единицы”)
5. `examples_json` - секция Examples (если есть)
6. `formula_md` - секция Formula (если есть)
7. `assumptions_md` - секция Assumptions (если есть)
8. `faq_json` - секция FAQ (если есть)
